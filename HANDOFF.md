...
## Где остановились
**Витрина главной заполнена городами — 2026-06-22.** `showcasePicks` в `src/layouts/HomePage.astro` ссылался на несуществующий контент → лента была пустой. Заполнен 2 опубликованными городами (`zagreb-travel-guide`, `dubrovnik-travel-guide`, `category=cities`); чип города локализован для ru. Тип массива расширен до union, чтобы спящие ветки restaurant/route оставались валидными. Витрина показывается на `/` и `/ru/`; LangSwitcher — ровно en+ru. Гейты `build`/`check`/`test`/`test:links`/`lint` зелёные.
